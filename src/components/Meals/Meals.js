...
export default class Meals extends Component {
            ...
            <div className="meals-container">
                {
                    meals.map((meal) =>
                        <React.Fragment key={meal.idMeal}>
                            <Meal meal={meal} favoriteMeals={favoriteMeals} context={context} />
                        </React.Fragment>
                    )
                }
            </div>
        ...
        if (favoriteMeals.length > 0) {
                ...
                <div className="fav-meals-container">
                    {
                        favoriteMeals.map((favMeal) =>
                            <React.Fragment key={favMeal.idMeal}>
                                <Meal meal={favMeal} favoriteMeals={favoriteMeals} context={context} />
                            </React.Fragment>
                        )
                    }
                </div>
            ...
            return (
                <div style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ cursor: 'pointer' }} onClick={this.handleBackButton}>Back</div>
                </div>
            )
        }
    }
    ...
    renderDetail(context, path) {
        if (path === "/fav") {
            return (
                this.renderFavMeals(context)
            )
        } else {
            return (
                this.renderMeals(context)
            )
        }
    }
        ...
        return (
            <React.Fragment>
                <MealContext.Consumer>
                    {(context) => (
                        this.renderDetail(context, path)
                    )}
                </MealContext.Consumer>
            </React.Fragment>
        )
